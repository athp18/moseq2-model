...
class TestCLI(TestCase):
    def test_count_frames(self):
        ...
        result = runner.invoke(count_frames, [input_file], catch_exceptions=False)
    ...
    def test_learn_model(self):
        ...
        result = runner.invoke(learn_model, train_params, catch_exceptions=False)

        assert result.exit_code == 0, "CLI Command did not successfully complete"
        assert os.path.exists(
            dest_file
        ), "Trained model file was not created or is in the incorrect location"
        assert os.path.exists(checkpoint_path)
        assert len(os.listdir(checkpoint_path)) == 5  # iters: 1, 3, 5, 7, 9

        assert os.path.exists(
            "data/test/train_val0_summary.png"
        ), "Training logLikes summary was not created."
    ...
    def test_kappa_scan(self):
        ...
        result = runner.invoke(
            kappa_scan_fit_models, kappa_scan_params, catch_exceptions=False
        )
